...
import { Check, ChevronRight, ChevronLeft, Timer as TimerIcon } from 'lucide-react';
  ...
  return (
    <div className="bg-primary/5 border border-primary/20 p-5 rounded-2xl flex items-center justify-between mt-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-xs font-bold text-text-muted mb-1">Timer Langkah Ini</p>
          <p className="text-2xl font-black text-primary font-mono tracking-wider">{displayTime}</p>
        </div>
      </div>
      <button 
        onClick={toggleTimer}
...
export default function CookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!recipe) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-bg-app">
        <h3 className="font-bold text-text-main text-xl mb-2">Resep Tidak Ditemukan</h3>
        <p className="text-text-muted mb-8">Resep tidak ditemukan atau sesi telah berakhir.</p>
  ...
  const currentStepData = recipe.steps.find(s => s.step_number === currentCookStep) || recipe.steps[0];
    ...
    <main className="flex-1 flex flex-col relative bg-white h-full overflow-y-auto">
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-md px-6 py-5 border-b border-border shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-bold text-text-main text-lg truncate pr-4">{recipe.title}</h2>
      ...
      <div className="p-6 flex-1 flex flex-col justify-center animate-in fade-in slide-in-from-right-4 duration-500" key={currentCookStep}>
